...
app.get("/api/users", async (req, res) => {
  try {
    const listOfAllUsersInfo = await UserMongoDB.find();
    const listOfAllUsers = listOfAllUsersInfo.map(user => ({ username: user.username, _id: user._id }));
    res.send(listOfAllUsers);
  } catch (err) {
...
app.post("/api/users", async (req, res) => {
  try {
    ...
    if (isExisted) {
      ...
      const newUser = new UserMongoDB({ username: userName, log: [] });
      const result = await newUser.save();
      res.send({ username: userName, _id: result._id });
    }
  } catch (err) {
    res.status(500).send({ error: 'Internal Server Error' });
  }
...
app.post("/api/users/:_id/exercises", async (req, res) => {
  try {
    ...
    userGet.log.push({ description, duration, date: dateInputted });
